...
#Import data
df <- read.csv('5.Pivort_10575.csv')
...
fil_df2 <- my_data[, colSums(is.na(my_data)) <= 3]
colnames(fil_df2) #Hereko

colnames(fil_df2) <- c('Alk','Carbon','Chloride', 'Days_since_PPT', 'Depth','Nitrogen', 'DO','pH','TNR','SC','Sulfate','Temp') #Rename haaneko



# extract the first row as a vector 
row1 <- fil_df2[, 5] #Flow or depth

# calculate the correlation between row1 and all other rows
correlations <- cor(fil_df2[, -5], row1, use = "pairwise.complete.obs")
...
ggplot(data = melted_cor, aes(x=Var2, y=Var1, fill=value, label = round(value, 2))) +
  ...
  ggtitle("Correlation Heatmap - Station 10575")
